...
function start()
	...
	load_bitmap("misc_graphics/particles/bomb_puff.png")
end
...
function logic()
	...
	if (boulder_tween1ed and not boulder_tween1_finished) then
		...
		append_tween(t, create_center_camera_tween(0.0, 24*TILE_SIZE, 5*TILE_SIZE, 150))
		new_tween(t)
...
function handle_boulder_collision(n)
	if (string.sub(get_entity_animation(get_player_id(0)), 1, 4) == "jump") then
		return
	end
...
function stop()
	...
	destroy_bitmap("misc_graphics/particles/bomb_puff.png")
end
