	...
	for (int i = 0; i < sz2; i++) {
		...
		for (int j = 0; j < sz; j++) {
			...
			if (tg->top_left.y+tg->size.h < entity_pos.y) {
				...
				if (tg->duration > 0) {
					Graphics::quick(false); // flush cache for this transform
				...
				for (int k = 0; k < sz; k++) {
					...
					if (curr_sheet < 0 || curr_sheet >= (int)tile_sheets.size()) {
						continue;
					}
				...
				if (tg->duration > 0) {
					Graphics::quick(true); // re-enable the cache
...
void Area_Manager::draw()
{
	if (cfg.water_shader) {
		...
		for (size_t i = 0; i < 1 && i < tile_sheets.size(); i++) {
...
std::list<Map_Entity *> Area_Manager::entity_is_colliding(Map_Entity *entity, General::Point<float> entity_pos, bool weapon)
{
	...
	for (int i = 0; i < sz; i++) {
		...
		if (curr_entity->collidable_get_type() == COLLIDABLE_BONES) {
			...
			if (General::distance(entity_pos.x, entity_pos.y, curr_pos.x, curr_pos.y) < 100) {
				if (!got_bones) {
					got_bones = true;
	...
	for (int i = 0; i < 256; i++) {
		...
		if (cfg.water_shader) {
			if (i == 0 && tile_sheets.size() > 0) {
				...
				al_set_target_bitmap(sheet->bitmap);
				al_set_blender(ALLEGRO_ADD, ALLEGRO_ONE, ALLEGRO_ZERO);
				Graphics::quick_draw(tile_sheets[tile_sheets.size()-1]->bitmap, 0, 0, 0);
				al_restore_state(&state);
			}
