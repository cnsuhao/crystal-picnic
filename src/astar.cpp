					...
					for (size_t point = 0; point < outline.size(); point++) {
						...
						for (int i = 0; i < dist; i += (min-0.5f)) {
							General::Point<float> p3(
								p1.x + (curr_pos.x + cosf(a) * i),
								p1.y + (curr_pos.y + sinf(a) * i) + General::BOTTOM_SPRITE_PADDING
							);
							General::Point<float> p4(
								p2.x + (curr_pos.x + cosf(a) * i),
								p2.y + (curr_pos.y + sinf(a) * i) + General::BOTTOM_SPRITE_PADDING
							);
