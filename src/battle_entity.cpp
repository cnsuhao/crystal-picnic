...
void Battle_Entity::ui_draw()
{
	...
	if (item_circle_state == ITEM_CIRCLE_ITEMS || item_circle_state == ITEM_CIRCLE_ITEMS_FADING_IN || item_circle_state == ITEM_CIRCLE_ITEMS_FADING_OUT) {
		for (int i = 0; i < 7; i++) {
			...
			float dx = center.x + cosf(angle) * ITEM_CIRCLE_RADIUS;
			float dy = center.y + sinf(angle) * ITEM_CIRCLE_RADIUS;
			float r, g, b;
			...
			Graphics::quick_draw(bmp->bitmap, al_map_rgba_f(r*alpha, g*alpha, b*alpha, alpha), w/2, h/2, dx, dy, 0.0f, 0);
		...
		for (size_t i = 0; i < names.size(); i++) {
			...
			float dx = center.x + cosf(angle) * ITEM_CIRCLE_RADIUS;
			float dy = center.y + sinf(angle) * ITEM_CIRCLE_RADIUS;
			Graphics::quick_draw(bmp->bitmap, al_map_rgba_f(alpha, alpha, alpha, alpha), w/2, h/2, dx, dy, 0.0f, 0);
...
// FIXME: up ignored
float Battle_Entity::get_closest_line_vertically(General::Point<float> pos, bool up, std::vector< std::vector< General::Point<int> > > &geometry,
