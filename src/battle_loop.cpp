...
static void draw_mini_battle_stats(ALLEGRO_COLOR tint, Battle_Player *p, int x, int y)
{
	Wrap::Bitmap *profile = p->get_profile_bitmap();
	Graphics::quick_draw(profile->bitmap, tint, x, y, 0);
...
void Battle_Loop::draw_bullet_time(General::Point<float> start, General::Point<float> end, float width)
{
	...
	Graphics::quick_draw(screen_copy_bmp->bitmap, X1, Y1, X2-X1, Y2-Y1, X1, Y1, 0);
	Shader::use(NULL);
}
	...
	for (bhi = 0; bhi < burrow_holes.size(); bhi++) {
		if (burrow_holes[bhi].end_time <= al_get_time()) {
			...
			if (burrow_particle_count % 5 == 0) {
				...
				float vx = cosf(angle) * 4;
				float vy = sinf(angle) * 4;
				p->data[2] = vx;
				p->data[3] = vy;
				p->data[4] = 6;
				p->data[5] = 0;
...
void Battle_Loop::draw_entities(int layer)
{
	...
	std::map<std::string, bool> drawn_this_kind;

	int sz = entities.size();
	for (int i = 0; i < sz; i++) {
		Battle_Entity *entity = entities[i];
		drawn_this_kind[entity->get_name()] = false;
	}

	for (int i = 0; i < sz; i++) {
		Battle_Entity *kind = entities[i];

		std::string this_kind = kind->get_name();

		if (drawn_this_kind[this_kind] == true) {
			continue;
		}

		drawn_this_kind[this_kind] = true;
		...
		for (int i = 0; i < sz; i++) {
			...
			if (player) {
				...
				if (skeleton) {
					...
					if ((entity->get_name() == "oldoak" && entity->get_attributes().hp <= 0) || (entity->get_attributes().hp > 0 && is_hurt)) {
...
void Battle_Loop::draw()
{
	...
	for (size_t layer = 0; layer < tiles.size(); layer++) {
		...
		for (int sheet = 0; sheet < num_sheets; sheet++) {
			...
			for (int t = 0; t < sz; t++) {
				...
				if (x+w >= 0 && y+h >= 0 && x < cfg.screen_w && y < cfg.screen_h) {
					...
					Graphics::quick_draw(sub->bitmap, x1, y1, x2-x1, y2-y1, _dx, _dy, 0);
				}
			}
		}
		...
		if ((int)layer == entity_layer) {
			...
			for (size_t i = 0; i < burrow_holes.size(); i++) {
				...
				if (frame > 3) frame = 3;
				Graphics::quick_draw(burrow_hole_bmps[frame]->bitmap, burrow_holes[i].pos.x-al_get_bitmap_width(burrow_hole_bmps[0]->bitmap)/2-get_top().x, burrow_holes[i].pos.y-al_get_bitmap_height(burrow_hole_bmps[0]->bitmap)-get_top().y+4, 0);
			}
			for (size_t i = 0; i < static_burrow_holes.size(); i++) {
				Graphics::quick_draw(burrow_hole_bmps[3]->bitmap, static_burrow_holes[i].pos.x-al_get_bitmap_width(burrow_hole_bmps[0]->bitmap)/2-get_top().x, static_burrow_holes[i].pos.y-al_get_bitmap_height(burrow_hole_bmps[0]->bitmap)-get_top().y+4, 0);
			}
...

	{
		...
		for (size_t i = 0; i < players.size(); i++) {
			...
			if (p) {
				if (cfg.low_graphics == false) {
					draw_mini_battle_stats(al_map_rgb(0, 0, 0), p, MINI_BATTLE_STATS_W*count+5*(count+1)+1, 5+1);
				}
				draw_mini_battle_stats(al_map_rgb(255, 255, 255), p, MINI_BATTLE_STATS_W*count+5*(count+1), 5);
				count++;
			}
		}
	}
...
void Battle_Loop::reload_graphics()
{
	...
	for (int i = 0; i < nlines; i++) {
		...
		else if (geo[i][0].x >= size.w-5) {
			geo[i][0].x = size.w + 10;
		}
		if (geo[i][pts-1].x <= 5) {
			geo[i][pts-1].x = -10;
		}
		else if (geo[i][pts-1].x >= size.w-5) {
			geo[i][pts-1].x = size.w + 10;
		}
