...
void Configuration::reset(void)
{
	...
	linear_filtering = true; // will be turned off if not needed in engine.cpp
...
bool Configuration::load(void)
{
	...
	const char *v_mute = al_get_config_value(acfg, "audio", "mute");
	if (v_mute) {
		mute = !strcmp(v_mute, "true");
		LOG("Read mute: " + std::string(v_mute));
	}
	else {
		LOG("Warning: mute not present.");
	}
...
bool Configuration::save(void)
{
	...
	al_set_config_value(acfg, "audio", "mute", mute ? "true" : "false");
