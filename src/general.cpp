...
static void my_draw_text(ALLEGRO_FONT *f, ALLEGRO_COLOR c, float x, float y, int flags, const char *str)
{
	...
	Graphics::draw_ustr(f, c, x, y, flags, u);
}
...
void load_fonts()
{
	...
	// Cache the whole translation in one go to avoid missing glyph bugs on some Androids

	ALLEGRO_CONFIG *sysconfig = al_get_system_config();
	al_set_config_value(sysconfig, "ttf", "skip_cache_misses", "true");
	al_set_config_value(sysconfig, "ttf", "cache_text", al_cstr(engine->get_entire_translation()));
	...
	float *outy)
{
	...
	float start_angle = asinf(0);
	float remain_angle = M_PI - start_angle;
	float want_angle = percent * remain_angle + start_angle;
	float angle = atan2(dist_y, dist_x);
	float real_start_x = cosf(angle + M_PI) * (diameter - distance) + sx;
	float real_start_y = sinf(angle + M_PI) * (diameter - distance) + sy;
...
void draw_poison_bubbles(General::Point<float> pos)
{
	...
	for (size_t i = 0; i < poison_bubbles.size(); i++) {
		...
		Graphics::quick_draw(poison_bmps[b.frame]->bitmap, dest.x-bmp_w/2, dest.y-bmp_h/2, 0);
	}
	Graphics::quick(was_quick);
}
