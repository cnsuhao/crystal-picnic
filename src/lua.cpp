...
static int c_draw_bitmap(lua_State *stack)
{
	...
	if (bmp) {
		Graphics::quick_draw(bmp->bitmap, x, y, flags);
	}
...
static int c_draw_tinted_rotated_bitmap(lua_State *stack)
{
	...
	if (bmp) {
		Graphics::quick_draw(bmp->bitmap, al_map_rgba_f(r, g, b, a), cx, cy, dx, dy, angle, flags);
	}
...
static int c_draw_bitmap_region(lua_State *stack)
{
	...
	if (bmp) {
		Graphics::quick_draw(bmp->bitmap, x, y, w, h, dx, dy, flags);
	}
...
static int c_set_clipping_rectangle(lua_State *stack)
{
	...
	bool was_quick = Graphics::is_quick_on(); // cache must be flushed to obey the clip
...
static int c_hold_bitmap_drawing(lua_State *stack)
{
	bool hold = lua_toboolean(stack, 1);
...
static int c_is_bitmap_drawing_held(lua_State *stack)
{
	lua_pushboolean(stack, al_is_bitmap_drawing_held());
	return 1;
}

static int c_load_animation_set(lua_State *stack)
{
	std::string dirname = lua_tostring(stack, 1);

	Animation_Set *a = new Animation_Set();
	a->load(dirname);

	lua_pushlightuserdata(stack, a);

	return 1;
}

static int c_destroy_animation_set(lua_State *stack)
{
	Animation_Set *a = (Animation_Set *)lua_touserdata(stack, 1);
...
void register_c_functions(lua_State *lua_state)
{
	...
	REGISTER_FUNCTION(hold_bitmap_drawing);
	REGISTER_FUNCTION(is_bitmap_drawing_held);
	REGISTER_FUNCTION(load_animation_set);
	REGISTER_FUNCTION(destroy_animation_set);
