			...
			if (e) {
				...
				if (camera) {
					if (panning_back_to_player) {
						...
						General::Point<float> pan(cosf(a) * pan_speed, sinf(a) * pan_speed);
							...
							if (found) {
								...
								if (!(unusable && (fabs(a) < five_deg || fabs(M_PI/2-a) < five_deg || fabs(M_PI-a) < five_deg || fabs(M_PI*3/2-a) < five_deg))) {
									...
									if (found) {
										...
										float a2 = atan2(tmp1.y - tmp2.y, tmp1.x - tmp2.x);
										float avg = (a1 + a2) / 2;
										float dxavg = General::sign(cosf(avg));
										float dyavg = General::sign(sinf(avg));
...
float Map_Entity::get_current_speed(void)
{
	...
	if (input[X] != 0 && input[Y] != 0) {
		curr_speed = sqrtf(curr_speed*curr_speed + curr_speed*curr_speed) / 2.0;
	}
	return curr_speed;
}
