...
bool Map_Loop::logic()
{
	...
	if (!(desired_offset == offset)) {
		...
		if (dist <= speed) {
			...
			offset.x += cosf(angle) * speed;
			offset.y += sinf(angle) * speed;
		}
	}
...
void Map_Loop::draw()
{
	...
	Graphics::quick_draw(map_bmp->bitmap, -offset.x, -offset.y, 0);
	...
	for (size_t i = 0; i < locations.size(); i++) {
		...
		Graphics::quick_draw(b->bitmap, l.pos.x-offset.x-al_get_bitmap_width(b->bitmap)/2, l.pos.y-offset.y-al_get_bitmap_height(b->bitmap)/2, 0);
	}
