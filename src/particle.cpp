	...
	if (battle_loop && bullet_time_len > 0.0f && bullet_time_start.x > General::SMALL_FLOAT) {
		...
		if (len > bullet_time_len) {
			start = General::Point<float>(
				bullet_time_start.x + cosf(angle) * (len-bullet_time_len),
				bullet_time_start.y + sinf(angle) * (len-bullet_time_len)
			);
			len = bullet_time_len;
		}
		else {
		...
		start.x += cosf(perp)*-2.0f;
		start.y += sinf(perp)*-2.0f;
		start.x -= topx;
		start.y -= topy;
		General::Point<float> end(
			pos.x+cosf(perp)*2.0f-topx,
			pos.y+sinf(perp)*2.0f-topy
		);
