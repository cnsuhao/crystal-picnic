...
void Player::handle_event(ALLEGRO_EVENT *event)
{
	...
	if (event->type != ALLEGRO_EVENT_KEY_UP && event->type != ALLEGRO_EVENT_KEY_DOWN && event->type != ALLEGRO_EVENT_JOYSTICK_AXIS && event->type != ALLEGRO_EVENT_JOYSTICK_BUTTON_UP && event->type != ALLEGRO_EVENT_JOYSTICK_BUTTON_DOWN) {
		return;
	}
