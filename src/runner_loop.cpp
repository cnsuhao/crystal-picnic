...
void Runner_Loop::extra_drawing_hook(int layer)
{
	...
	if (layer == PLAYER_LAYER) {
		...
		for (size_t i = 0; i < shield_positions.size(); i++) {
			if (shield_positions[i].x < pp.x-cfg.screen_w/2-100 || shield_positions[i].x > pp.x+cfg.screen_w/2)
				continue;
			Graphics::quick_draw(shield_bmp->bitmap, shield_positions[i].x-top.x, shield_positions[i].y-top.y-al_get_bitmap_height(shield_bmp->bitmap), 0);
		}
		...
		for (size_t i = 0; i < coins.size(); i++) {
			...
			Graphics::quick_draw(bmp->bitmap, coins[i].x-top.x, coins[i].y-top.y+coin_offset(), 0);
		}
		...
		if (shields > 0 && dead_time > al_get_time()) {
			...
			int x1 = x - 32;
			int y1 = y - 48;
			Graphics::quick_draw(shield_barrier->get_current_animation()->get_current_frame()->get_bitmap()->get_bitmap()->bitmap,
		...
		for (unsigned int i = 0; i < active_birds.size(); i++) {
			...
			if (b.x < pp.x-cfg.screen_w/2-150 || b.x > pp.x+cfg.screen_w/2)
				continue;
			Graphics::quick_draw(bird_bmp->bitmap, b.x-top.x, b.y-top.y, 0);
		}
		Graphics::quick(was_quick);
	}
	...
	if (layer == (bees_on_top ? LAST_LAYER : BEE_LAYER)) {
		...
		float add_y = -10 + 10 * sinf(bee_sine_wave);
		float add_x = -32 + 10 * cosf(bee_cosine_wave);
		if (won) {
...
void Runner_Loop::draw(void)
{
	...
	Graphics::quick_draw(gradient->bitmap, 0, 0, 0);

	// draw clouds
	for (size_t i = 0; i < clouds.size(); i++) {
		Graphics::quick_draw(clouds[i]->bitmap->bitmap, clouds[i]->x, clouds[i]->y, 0);
	}
	...
	for (int i = 0; i < shields; i++) {
		Graphics::quick_draw(shield_bmp->bitmap, 10+i*al_get_bitmap_width(shield_bmp->bitmap)+i*5, 10, 0);
	}
	...
	for (int i = 0; i < NUM_PLATFORMS; i++) {
		...
		//if (General::rand() % 2 == 0 || since_bird >= 3) {
			...
			if (pattern == 1 || pattern == 2) {
				...
				float dist = centerx;
				for (int i2 = 0; i2 < 5; i2++) {
					int x = cosf(angle) * dist + centerx;
					int y = sinf(angle) * dist + centery;
					c.x = startx+x;
					c.y = platy+y-coin_h;
					coins.push_back(c);
					angle += M_PI/12;
		...
		if (type == 0) {
			...
			float avg_dist = sqrtf((dist*dist)/2); // if both sides are equal
			int half = avg_dist / 2;
	...
	won(false)
{
	...
	atlas = atlas_create(
		General::texture_size, General::texture_size, ATLAS_REPEAT_EDGES, 1, true
	);
