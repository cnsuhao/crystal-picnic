...
void Skeleton::recurse(General::Point<float> offset, Link *l, ALLEGRO_TRANSFORM t, bool draw_it, bool flip_it, int layer, ALLEGRO_COLOR tint, bool reversed)
{
	...
	if (l->part->get_layer() == layer) {
		if (draw_it) {
			...
			al_transform_coordinates(&l->final_trans, &x1, &y1);
			al_transform_coordinates(&l->final_trans, &x2, &y2);
			al_transform_coordinates(&l->final_trans, &x3, &y3);
			al_transform_coordinates(&l->final_trans, &x4, &y4);

			Graphics::quick_cache(bitmap->bitmap, tint, x1, y1, x2, y2, x3, y3, x4, y4, 0);
...
Skeleton::~Skeleton()
{
	...
	if (atlas) {
		resource_manager->release_atlas(filename);
	}
}
...
bool Skeleton::load()
{
	...
	if (resource_manager->check_atlas_exists(filename)) {
		atlas_exists = true;
		atlas = resource_manager->reference_atlas(filename);
	}
	else {
		atlas_exists = false;
		atlas = atlas_create(512, 512, 0, 1, false, true);
	}
	...
	if (atlas_exists == false) {
		...
		resource_manager->set_atlas(filename, atlas);
	}
